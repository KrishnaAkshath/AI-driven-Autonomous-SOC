...
<style>
    ...
    .oauth-google { border-color: rgba(66, 133, 244, 0.5); }
    .oauth-google:hover { background: rgba(66, 133, 244, 0.15); }
    
    .oauth-apple { border-color: rgba(255, 255, 255, 0.3); }
    .oauth-apple:hover { background: rgba(255, 255, 255, 0.1); }
...
<div class="login-container">
    <div class="logo-area">
        ...
        <p style="color: #8B95A5; margin: 0.3rem 0 0 0; font-size: 0.9rem;">Autonomous Security Operations</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
# OAuth buttons
st.markdown("""
<a class="oauth-btn oauth-google" href="#">
...
if st.session_state.otp_sent and st.session_state.pending_user:
    st.markdown("""
        <div style="text-align: center; margin-bottom: 1rem;">
            <p style="color: #00D4FF; font-size: 1.1rem; margin: 0;">Verification Required</p>
            <p style="color: #8B95A5; font-size: 0.9rem; margin: 0.3rem 0 0 0;">Enter the 6-digit code sent to you</p>
        </div>
    """, unsafe_allow_html=True)
    
    otp_code = st.text_input("Verification Code", placeholder="000000", key="otp_input", max_chars=6)
    
    trust_device = st.checkbox("Trust this device for 30 days", value=True)
    ...
    with col1:
        if st.button("Verify", type="primary", use_container_width=True):
            if otp_code and len(otp_code) == 6:
                success, message = verify_otp(st.session_state.pending_user['email'], otp_code)
                if success:
    ...
    with col2:
        if st.button("Resend Code", use_container_width=True):
            ...
            success_email, _ = send_otp_email(st.session_state.pending_user['email'], otp)
            success_tg, _ = send_otp_telegram(otp)
            if success_email or success_tg:
    ...
    if st.button("Sign In", type="primary", use_container_width=True):
        if email and password:
            ...
            if success and user:
                ...
                if is_device_trusted(email, device_id):
                    ...
                    success_email, msg_email = send_otp_email(email, otp, email)
                    success_tg, msg_tg = send_otp_telegram(otp)
                    
                    if success_email or success_tg:
                        st.session_state.otp_sent = True
                        st.session_state.pending_user = user
                        st.info(f"Verification code sent! {msg_email if success_email else msg_tg}")
                        st.rerun()
                    else:
                        # Fallback: allow login if notifications fail
                        st.session_state.auth_token = user['token']
                        st.warning("2FA unavailable. Logged in directly.")
                        st.switch_page("pages/1_Dashboard.py")
            else:
                st.error(message)
        else:
